...
export class LanguageComponent  implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private navParams: NavParams, private modalCtrl: ModalController, private systemLanguage: SystemLanguageService, public newLanguage: LanguageSaveService) {
  ...
  public handleCreate(){
    ...
    this.saveCommandSubscription = this.newLanguage.createCommand.subscribe(
      command => {
  ...
  private async loadingFormCtrl(){
    ...
    this.keyPairValueList = await this.systemLanguage.getSelectedLanguageKeyPairValueList(this.language.code);
    this.keyPairValueList = this.keyPairValueList.map(kv => {kv.value = kv.value === undefined ? "" : kv.value; return kv});
