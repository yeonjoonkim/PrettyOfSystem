...
export class RoleComponent implements OnInit {
  ...
  constructor(private global: GlobalService, private systemRole: SystemRoleService, private navParams: NavParams) {
    this.getParams();
  }
  ...
  public async onClickSaveRoleConfiguration(){
    let hasSelectedConfig: boolean = this.role.accessLevel.isSystemAdmin  || this.role.accessLevel.isAdmin || this.role.accessLevel.isManager || this.role.accessLevel.isEmployee || this.role.accessLevel.isReception;
  ...
  private async presentErroMsg(hasDescription: boolean, hasSelectedConfig: boolean): Promise<void>{
    let errormsg: string = !hasDescription    ? await this.global.language.transform('message.error.descriptionfield') :
                           !hasSelectedConfig ? await this.global.language.transform('message.error.config')
                           : '';
