...
export class LanguageService {
  ...
  constructor(
    ...
    private systemLanguageRepository: SystemLanguageRepositoryService,
  ...
  public async getDefaultKeyPairValueList(): Promise<IPairKeyValue[]> {
    ...
    let keyPairValueList: IPairKeyValue[] = this.languagePackage.getKeyPairValue(
      key.used,
      defaultLanguage.package
    );
  ...
  public async validateNewKeyPairValue(keyPairValue: IPairKeyValue) {
    ...
    let validated: IAddLanguageTransformSaveCommand = {
      ...
      isTransformKeyValueFormat:
        this.textTransform.setLanguageTransformCodeList(keyPairValue.key.toLowerCase()).length ===
        3,
    };
  ...
  public async editLanguagePackage(
  ...
  private async updateLanguagePackage(
    ...
    languages.forEach(async language => {
      ...
      for (let translatedLanguageCode in translated) {
        if (languageCode === translatedLanguageCode) {
          let keyPairValue: IPairKeyValue = {
            key: keyValue,
            value: translated[translatedLanguageCode],
          };
          language.package = this.languagePackage.updateKeyValuePackage(
            language.package,
            keyPairValue
          );
  ...
  public async editSelectedPackage(
    selectedLanguageCode: string,
    keyPairValue: IPairKeyValue
  ): Promise<void> {
    let language = (await this.getLanguageSelection()).filter(
  ...
  public async getLanguageSelection(): Promise<ILanguageSelection[]> {
    ...
    let storage: ILanguageSelection[] = isNull
      ? await this.getLanguageSelectionResult()
      : await this.storage.getLanguageSelection();
    if (isNull) {
  ...
  public async getLanguageSelectionResult(): Promise<ILanguageSelection[]> {
    let currentSelection = await this.storage.getLanguageSelection();
    return currentSelection
      ? currentSelection
      : lastValueFrom(this.systemLanguageRepository.getLanguageSelectionResult());
  }
