...
export class LanguageSaveService {
  ...
  public createCommand: Observable<ICreateNewPackageCommand | undefined> =
  ...
  public async saveNewTitleTransformValue(command: IPairKeyValue): Promise<boolean> {
    let validated: IAddLanguageTransformSaveCommand =
      await this.global.language.validateNewKeyPairValue(command);
  ...
  private async translateCurrentKeyPairValue(command: ICreateNewPackageCommand) {
    ...
    let isDescriptionFormat: boolean = this.global.textTransform.isDescriptionFormat(
      command.currentKeyPair.value
    );
    let translateCriteria: ILanguageTranslatedCriteria = {
      code: [command.code],
      name: [command.translateTo],
      isTitle: !isDescriptionFormat,
    };
    ...
    let translatedResult: ILanguageTranslateItem =
      await this.translate.getTranslatedLanguagePackageWithOutLoading(
        command.currentKeyPair.value,
        translateCriteria
      );
  ...
  private async successTranslate(
    ...
    command.currentKeyPair.value =
      typeof translatedResult.translated[command.code] === 'string'
        ...
        : command.currentKeyPair.value;
    command.newPackage = this.languagePackage.updateKeyValuePackage(
      command.newPackage,
