...
export class SystemMenuCategoryAddService {
  ...
  constructor(private global: GlobalService, private systemMenuRepository: SystemMenuRepositoryService) {
  }
  ...
  public async processSaveNewSystemMenuCategory(newCategory: IMenuCategory): Promise<void>{
    newCategory.name = this.global.textTransform.getTransformObjectKeyValue(this.prefixedCategoryOjbectName, newCategory.description);
  ...
  public async getTranslatedCategoryDescription(newCategory: IMenuCategory): Promise<ILanguageTranslateItem>{
    ...
    let result: ILanguageTranslateItem = await this.global.languageTranslate.getTranslatedLanguagePackage(newCategory.description, translateCriteria);
    return result;
  }
  ...
  public async getTranslatedContentDescription(newContent: IMenuContent): Promise<ILanguageTranslateItem>{
    let translateCriteria = await this.global.language.getAllLanguageTranslateCriteria();
    translateCriteria.isTitle = true;
    let result: ILanguageTranslateItem = await this.global.languageTranslate.getTranslatedLanguagePackage(newContent.description, translateCriteria);
    return result;
  }
