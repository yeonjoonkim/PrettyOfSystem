...
export class SystemMenuCategoryService {
      ...
      if(previous && edited.icon && edited.description){
        if(needLanguageTranslate){
          ...
          if(!result.isEmpty){
            ...
            edited.description = this.global.language.getDefaultLanguageDescription(result.translated);
            edited.name = this.global.textTransform.getTransformObjectKeyValue(this.systemMenuCategoryAddService.prefixedCategoryOjbectName, edited.description);
  ...
  private async addNewSystemMenuContent(newCategoryContent: IMenuContent, systemMenuCategory: IMenuCategory){
    ...
    if(!result.isEmpty){
      newCategoryContent.description = this.global.language.getDefaultLanguageDescription(result.translated);
      newCategoryContent.name = this.global.textTransform.getTransformObjectKeyValue(this.systemMenuCategoryAddService.prefixedCategoryContentObjectName, newCategoryContent.description.toLowerCase());
  ...
  private async updateSystemMenuContent(previousContent: IMenuContent, newContent: IMenuContent, systemMenuCategory: IMenuCategory){
    ...
    if(needTranslate){
      ...
      if(!result.isEmpty){
        newContent.description = this.global.language.getDefaultLanguageDescription(result.translated);
        newContent.name = this.global.textTransform.getTransformObjectKeyValue(this.systemMenuCategoryAddService.prefixedCategoryContentObjectName, newContent.description.toLowerCase());
