...
export class AddressComponent implements OnInit, OnChanges {
  ...
  private handleOnChangeDefaultState(changes: SimpleChanges) {
    ...
    let previousValue =
      changes[this.onChangeOption.defaultState]?.previousValue;
    let valueChanged =
  ...
  private setDefault() {
    ...
    this.address.suburb = isValidated
      ? this.address.suburb
      : this.defaultSuburb;
    this.filterOption = this.postcode.setPostCodeFilterOption(
      this.address.state
    );
    this.onChangeSuburb();
  }
  ...
  public onChangeSuburbInput(suburb: string) {
    ...
    this.autoCompleteFilter.suburb = this.filterOption.postCodeList
      ...
      .filter(
        (sub, index, array) =>
          array.indexOf(sub) === index && sub.indexOf(suburb) !== -1
      );
  }
