...
export class AustraliaPostCodeService {
  ...
  constructor(public nsw: NswService, public qld: QldService, public wa: WaService, public vic: VicService, public tas: TasService, public sa: SaService, public nt: NtService, public act: ActService) { }
  ...
  public getSelectedPostCodeList(state: Constant.AustraliaStateType): IPostCodeItem[]{
    return state === Constant.State.AustraliaType.NSW ? this.nsw.data
         : state === Constant.State.AustraliaType.QLD ? this.qld.data
         : state === Constant.State.AustraliaType.WA ? this.wa.data
         : state === Constant.State.AustraliaType.NT ? this.nt.data
         : state === Constant.State.AustraliaType.SA ? this.sa.data
         : state === Constant.State.AustraliaType.TAS ? this.tas.data
         : state === Constant.State.AustraliaType.ACT ? this.act.data
         : this.qld.data;
  }
